...
class Controller_Common extends Controller_Template
{
	...
	public function before()
	{
		...
		foreach (\Auth::verified() as $driver)
		{
			if (($id = $driver->get_user_id()) !== false)
			{
                ...
                $clients = Model_Client::find('all', ["where" => [ ['user_id', '=', $this->current_user->id ] ] ]);
                $item = null;
        ...
        $firstDayLastMonth = new \Carbon\Carbon('first day of last month');
        $lastDayLastMonth  = new \Carbon\Carbon('last day of last month');
        $firstDayThisMonth = new \Carbon\Carbon('first day of this month');
        $lastDayThisMonth  = new \Carbon\Carbon('last day of this month');

        $this->data_payload['firstDayLastMonth'] = $firstDayLastMonth;
        $this->data_payload['lastDayLastMonth'] = $lastDayLastMonth;
        $this->data_payload['firstDayThisMonth'] = $firstDayThisMonth;
        $this->data_payload['lastDayThisMonth'] = $lastDayThisMonth;

        $lastMonth = new \Carbon\Carbon('last month');
        $last_Month_literal = $lastMonth->format('F Y'); // format l, j M Y H:i:s
        $thisMonth = new \Carbon\Carbon('this month');
        $this->data_payload['lastMonth'] = $lastMonth;
        $this->data_payload['thisMonth'] = $thisMonth;

        $this->data_payload['daysRangeThisMonth'] = $this->get_days_range();
        $this->data_payload['daysRangeLastMonth'] = $this->get_days_range("first day of last month", "first day of last month");
		...
		if( Auth::check() ){
			$this->data_payload['chats'] = Model_Chat::find('all', array('where' => array( 
                ...
                array('to_user_id','=' , $this->current_user->id),
													)));
            ...
            $this->data_payload['activities'] = Model_Activitylog::find('all',
                [
                    //'related' => ['client', 'category'],
                    'where' => [
                        ['deleted_at' => null, ] // $this->employee_user->company_id
                    ],
                    'order_by'=> ["created_at" => "desc"]
                ]);
            $this->data_payload['activity_logs'] = Model_Activitylog::find('all',
                [
                    //'related' => ['client', 'category'],
                    'where' => [
                        ['deleted_at' => null], // $this->employee_user->company_id
            ...
            if(Auth::member(60) == 60  ){
                $this->data_payload['activities'] = Model_Activitylog::find('all',
                    [
                        //'related' => ['client', 'category'],
                        'where' => [
                            ['deleted_at' => null, "created_by" => $this->current_user->id ] // $this->employee_user->company_id
                        ],
                        'order_by'=> ["created_at" => "desc"]
                    ]);
                $this->data_payload['activity_logs'] = Model_Activitylog::find('all',
		...
		View::set_global('current_client', $this->current_client);
    ...
    public function get_days_range($min_date_string = "first day of this month",$max_date_string= "last day of this month"){
	    $temp_days_range = array();
        $_first_day = new Carbon( $min_date_string);
        $_last_day = new Carbon($max_date_string);
        $_date_hldr = $_first_day;

	    while ( $_date_hldr <= $_last_day ){
            $temp_days_range[] = $_date_hldr->format('d');
            $_date_hldr->addDay(1);
        }

	    return $temp_days_range;
    }
